...
void Jogo::imprimirObjetosTela()
{
    ...
    for (i = 0; i < total; i++)
    {
        ...
        if (pista->getObjetos()[i]->getAtivo())
            draw_sprite(buffer, pista->getObjetos()[i]->getSprite() ,pista->getObjetos()[i]->posicao.x, pista->getObjetos()[i]->posicao.y);
    }
}
