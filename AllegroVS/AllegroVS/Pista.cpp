...
Pista::Pista()
{
	...
	player1 = NULL;
	player2 = NULL;
	//inicializa todos os sprites como inativos
	for (i = 0; i < 400; i++)
		VetorSprites[i].ativo = false;
...
CORBA::Boolean Pista::Refresh( CORBA::Long tempo )//Testa as colisoes e altera a posAtual da pista
{
	Dimensao dimAux;
	if (this->pausado)
		return false;
    ...
    for (i = 0; i < this->objetos.size(); i++) //verifica se os objetos ativos nao foram comidos pela pista por baixo
    {
        ...
        if(aux1->getAtivo())
        {
            if (aux1->getTipo() == OBSTACULO)
            {
				...
				VetorSprites[i].posicao.x = dimAux.x;
				VetorSprites[i].posicao.y = dimAux.y;
			...
			if (VetorSprites[i].ativo)
				VetorSprites[i].ativo = aux1->getAtivo();
        }
    }
    return true;
}
...
void Pista::testarColisoes(Objeto* obj)
{
    ...
    for (i = 0; i < this->objetos.size(); i++)
    {
		...
		if (VetorSprites[i].ativo)
			VetorSprites[i].ativo = this->objetos[i]->getAtivo();
    }
}

void Pista::alterarAceleracao(CORBA::Long aceleracao)
{
    unsigned int i;
    for (i = 0; i < this->objetos.size(); i++)
    {
...
void Pista::adicionarObjeto(Objeto* obj) throw(Erros)
{
    ...
    if (!colidiu)
    {
		...
		ICorba::Sprite spriteAux;
		spriteAux.posicao.x = obj->getPosicao().x;
		spriteAux.posicao.y = obj->getPosicao().y;
		spriteAux.tamanho.x = obj->getTamanho().x;
		spriteAux.tamanho.y = obj->getTamanho().y;
		strcpy(spriteAux.strImagem,obj->getStrSprite().c_str());
		spriteAux.ativo = obj->getAtivo();
		spriteAux.tipo = obj->getTipo();
		this->VetorSprites[numVetorSprite] = spriteAux;
		numVetorSprite++;
...
void Pista::enviar_tecla(CORBA::Long tecla, CORBA::Long player){ //envia para ser processado para o servidor qual tecla foi precionada
	Personagem* aux = NULL;
	if (player == 1)
	{
		aux = player1;
...
void Pista::MorrerPersonagem(CORBA::Long player, CORBA::Long temporizador){
	if (player == 1)
	{
...
void Pista::setGodMod(CORBA::Long player){
	if (player == 1)
	{
...
CORBA::Boolean Pista::getAtivo(CORBA::Long player)
{
...
void Pista::mudarImagemPersonagem(CORBA::Long player){
	if (player == 1)
	{
...
CORBA::Long Pista::getTempoFim(CORBA::Long player){
	if (player == 1)
	{
...
CORBA::Long	Pista::getObstaculosAtingidos(CORBA::Long player){
	if (player == 1)
	{
...
CORBA::Long	Pista::getScore(CORBA::Long player){
	if (player == 1)
	{
...
void Pista::testarColisoesPersonagem(CORBA::Long player){
	Personagem* aux = NULL;
	Dimensao dimAux;
	if (player == 1)
	{
...
char* Pista::getPathImg()
{ 
	return this->pathImg;
}

CORBA::Boolean Pista::getAcabarJogo()
{
	return this->AcabarJogo;
}
void Pista::setAcabarJogo(CORBA::Boolean acabarJogo)
{
	this->AcabarJogo = acabarJogo;
	//if (acabarJogo)
	//	this->terminar();
}
CORBA::Long Pista::criarPersonagem()
{
	int retorno;
	if (numero_Personagens == 1)
	{
		player2 = new Personagem(true,"resources/venom0",dimensao(90,400),dimensao(50,119),20,3,false);
		player2ID = this->numVetorSprite;
	...
	cout << "\nAdicionado o personagem " << this->numero_Personagens;

	return retorno;
}
CORBA::Boolean Pista::getPausar()
{
	return this->pausado;
}
CORBA::Long Pista::getPosAtual()
{
	return this->posAtual;
}
void Pista::setPausar(CORBA::Boolean pausado)
{
	this->pausado = pausado;
}
::ICorba::IPista::vetor_objetos_slice* Pista::getObjetos()
{
	return VetorSprites;
}
CORBA::Long Pista::getID(CORBA::Long player)
{
	if (player == 1)
		return player1ID;
	else
...
::ICorba::Sprite Pista::getPersonagem(CORBA::Long player)
{
	ICorba::Sprite spriteAux;
	Personagem* obj = NULL;
...
CORBA::Boolean Pista::criarPista( const char* caminho, const ::ICorba::Dimensao& dimValida )
{
	if (this->numero_Personagens == 2)
		this->terminar();
	if (this->numero_Personagens == 0) {
	this->numVetorSprite = 0;
    this->numero_Obstaculos = 0;
    this->posAtual = -3000;
    this->AcabarJogo = false;
    this->DoisJogadores = true;
    this->imgAtual = 1;
    this->strcomecoCaminho = caminho;
	this->dimValida.x = dimValida.x;
	this->dimValida.y = dimValida.y;
    this->mudarImagem();
	}
	return true;
}
CORBA::Long Pista::getNumeroJogador()
{
	return numero_Personagens;
}

CORBA::Long Pista::getMunicao(CORBA::Long player)
{
	if (player == 1)
...
CORBA::Double Pista::getVelocidadein(CORBA::Long player)
{
	if (player == 1)
...
void Pista::iniciarJogo(CORBA::Long estagio)
{
	if (numero_Personagens == 0){ //Executa apenas 1 vez
		if (estagio == 1)
		{
