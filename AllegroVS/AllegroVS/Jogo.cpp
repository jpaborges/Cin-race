...
Jogo::Jogo(ICorba::Dimensao tela, ICorba::IPista* pista_servidor)
{
	this->pista = pista_servidor;
	imgPista = NULL;
	entrada1 = NULL;
	help = NULL;
	tiro = NULL;
...
Imagem* Jogo::criarImagem(ICorba::Sprite obj){
	BITMAP* retorno = NULL;
...
bool Jogo::iniciarAllegro(std::string titulo,ICorba::Dimensao tela )
{
    bool retorno = true;
...
{
    unsigned int i, total,id1,id2;
	BITMAP* bmpAuxiliar;
	ICorba::IPista::vetor_objetos_slice* objetos =  this->VetorSprites;
	id1 = pista->getID(1);
	id2 = pista->getID(2);
	objetos[id1] = pista->getPersonagem(1);
	objetos[id2] = pista->getPersonagem(2);

	total = 400;//O tamanho do vetor de sprites
	...
	//Texto Player 1
      ...
      textprintf_ex(buffer, font, 700, 330, makecol(255, 255, 255),
                          -1, "Speed %.2f",pista->getVelocidadein(1));
  ...
  //Texto Player 2
     ...
     textprintf_ex(buffer, font, 700, 230, makecol(255, 255, 255),
                          -1, "Speed %.2f",pista->getVelocidadein(2));
     textprintf_ex(buffer, font, 700, 260, makecol(255, 255, 255),
                          -1, "Shoots %d",pista->getMunicao(2));
	
	for (i = 0; i < total; i++)	
    {
		//printf("\nteste %s %d %d",objetos[i].strImagem,objetos[i].tipo,objetos[i].ativo);
		if (objetos[i].tipo == PERSONAGEM)
		{
			if ( (i != id1) && (i!=id2))
		...
		if (objetos[i].ativo)
		{
		   ...
		   else printf("\nErro na img %s",objetos[i].strImagem);
		}
    }
}
...
void Jogo::atualizarImgPista(){
	...
	if ((imgPista == NULL) || (imgPista->path.compare(pista->getPathImg()) != 0))
	{
		...
		aux = load_bitmap(pista->getPathImg(), NULL);
...
ICorba::Dimensao Jogo::dimensao(int x, int y)
{
	ICorba::Dimensao ret;
	ret.x = x;
	ret.y = y;
...
void Jogo::novoJogo(int estagio)
{
	...
	else
	{
		...
		if (numPersonagem == 1)
		{
			if (waitp2 == NULL)
			{
				waitp2 = create_bitmap(242,47);
				waitp2 = load_bitmap("resources/wait.bmp",NULL);
			}
			draw_sprite(buffer, waitp2, 0, 0);
			draw_sprite(screen, buffer, 0, 0);
		}
		...
		while (pista->getNumeroJogador() != 2)//Espera Ocupada
		{
			...
			if (key[KEY_T]){ //entra no modo teste
				rest(100);
		...
		while (!pista->getAcabarJogo() && !veiodeEsc)
		{
				...
				if (pista->getAtivo(numPersonagem))
				{
					if (temporizador - tempoC1 >= INTERVALO_MOVIMENTO)
					{
						...
						if (count > 2)
						{
							...
							if (temporizador - tempoImg1 >= (10 + INTERVALO_MOVIMENTO + INTERVALO_MOVIMENTO/pista->getVelocidadein(numPersonagem)))
							{
