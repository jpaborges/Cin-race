...
{
    status = SPLASH;
    BITMAP* entrada1 = create_bitmap(800,600);
...
void Jogo::imprimirObjetosTela()
{
    ...
    for (i = 0; i < total; i++)
    {
        ...
        if (pista->objetos[i]->ativo)
            draw_sprite(buffer, pista->objetos[i]->sprite ,pista->objetos[i]->posicao.x, pista->objetos[i]->posicao.y);
    }
}

void Jogo::contagemRegressiva()
{
    BITMAP* tres = create_bitmap(72,98);
    tres = load_bitmap("resources/3.bmp",NULL);
    BITMAP* dois = create_bitmap(74,100);
    dois = load_bitmap("resources/2.bmp",NULL);
    BITMAP* um = create_bitmap(73,100);
    um = load_bitmap("resources/1.bmp",NULL);
    BITMAP* ready = create_bitmap(274,97);
...
void Jogo::novoJogo(int estagio)
{
    ...
    if (estagio == 1)
    {
        ...
        pista->dimValida = dimensao(600,600);
    }
    else
    {
        pista = new Pista("resources/pista2_");
        pista->dimValida = dimensao(500,600);
    }
    ...
    //Tiro
    pista->adicionarObjeto(personagem1->tiro);
    pista->adicionarObjeto(personagem2->tiro);
    ...
    while (!pista->AcabarJogo)
    {
        ...
        if ( veiodeEsc || pista->AcabarJogo || temporizador >= TIME_LIMITE)
        {
            pista->AcabarJogo = true;
        }
        else
        {
            if (key[KEY_J])
                personagem2->godmod = true;  //cheat para nao colidir

            if (key[KEY_K])
                personagem1->godmod = true;  //cheat para nao colidir

            if (personagem1->ativo)
            {
                ...
                if (temporizador - tempoC1 >= INTERVALO_MOVIMENTO)
                {
                    ...
                    if (count > 2)
                    {
                        ...
                        personagem1->posicao.y += VELOCIDADE_PISTA;

                        if (temporizador - tempoImg1 >= (10 + INTERVALO_MOVIMENTO + INTERVALO_MOVIMENTO/personagem1->velocidade))
                        {
            ...
            if (personagem2->ativo)
            {
                if (temporizador - tempoC2 >= INTERVALO_MOVIMENTO)
                {
                    ...
                    if (count > 2)
                    {
                        ...
                        personagem2->posicao.y += VELOCIDADE_PISTA;
                        if (temporizador - tempoImg2 >= (10 + INTERVALO_MOVIMENTO + INTERVALO_MOVIMENTO/personagem2->velocidade))
                        {
    ...
    {
        if (personagem1->ativo)
            personagem1->Morrer(temporizador);

        if (personagem2->ativo)
            personagem2->Morrer(temporizador);
...
bool Jogo::exibirPontuacao()
{
    ...
    textprintf_ex(buffer, font, 212, 347, makecol(255, 255, 255),-1, "%d s",personagem2->tempoFim/1000);
    textprintf_ex(buffer, font, 493, 347, makecol(255, 255, 255),-1, "%d s",personagem1->tempoFim/1000);
    textprintf_ex(buffer, font, 263, 389, makecol(255, 255, 255),-1, "%d",personagem2->obstaculoAtingidos);
    textprintf_ex(buffer, font, 544, 389, makecol(255, 255, 255),-1, "%d",personagem1->obstaculoAtingidos);
...
{
    BITMAP* help = create_bitmap(800,600);
